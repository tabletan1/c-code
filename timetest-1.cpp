...
void RunList(const char filename[])
{
	...
	char command, line[200], temp;
	int data;
	inf.getline(line, 200, '\n');
	List<int> *list = new List<int>();
	
	while((command = inf.get()))
	{
		inf >> data >> temp;

		if (command == 'i')
			list->insert(data, list->zeroth());
		else if (command == 'd')
			list->remove(data);
	}
}
...
void RunCursorList(const char filename[])
{
	...
	int data;
	inf.getline(line, 200, '\n');
	vector<CursorNode<int> > cur(500001);
...
void RunStackAr(const char filename[])
{
	...
	int data;
	inf.getline(line, 200, '\n');
	StackAr<int> *sa = new StackAr<int>(500001);
...
void RunQueueAr(const char filename[])
{
	...
	int data;
	inf.getline(line, 200, '\n');
	Queue<int> *qa = new Queue<int>(500001);
...
void RunSkipList(const char filename[])
{
	...
	int data;
	inf.getline(line, 200, '\n');
	SkipList<int> *sl = new SkipList<int>(-1, 500001);
